        ...
        **kwargs
        ...
        with tqdm(total=total_steps, disable=True, desc='Appearance (opt): optimizing') as pbar:
            for step in range(total_steps):
                ...
                loss_gs = loss_utils.l1_loss(rend_gs, input_images) + (1 - loss_utils.ssim(rend_gs, input_images)) + loss_utils.lpips(rend_gs, input_images) + dreamsim_model(rend_gs, input_images).mean()
        ...
        **kwargs
        ...
        for i, (t, t_prev) in enumerate(tqdm(t_pairs, desc="Sampling", disable=not verbose)):
            if t > start_t:
                ...
                out = self.sample_once_opt_delta_v(model, slat_decoder_gs, slat_decoder_mesh, dreamsim_model, learning_rate, input_images, extrinsics, intrinsics, sample, t, t_prev, cond, **kwargs)
                sample = out.pred_x_prev
                ret.pred_x_t.append(out.pred_x_prev)
                ret.pred_x_0.append(out.pred_x_0)
        ret.samples = sample
    ...
    def sample_opt_delta_v(
        ...
        **kwargs
        ...
        Returns:
            ...
            - 'pred_x_0': a list of prediction of x_0.
        """
        return super().sample_opt_delta_v(model, slat_decoder_gs, slat_decoder_mesh, dreamsim_model, apperance_learning_rate, start_t, input_images, extrinsics, intrinsics,noise, cond, steps, rescale_t, verbose, neg_cond=neg_cond, cfg_strength=cfg_strength, cfg_interval=cfg_interval, **kwargs)
